 ...
 * @param onChatRequested how a "go to chat" intent is satisfied for this tab. Defaults to
 *   switching to the Chat tab; the Chat tab itself passes its own push behaviour.
...
class TabRoutingNavigator(
    ...
    private val onChatRequested: (initialMessage: String?) -> Unit = { selectTab(AppTab.CHAT) },
    private val onRootBack: () -> Boolean = { false }
) : Navigator by base {

    override fun navigateToChat(initialMessage: String?) {
        onChatRequested(initialMessage)
    }
